function Test-IntuneDeviceHealthScript {
    <#
    .SYNOPSIS
        Checks if Intune Device Health Scripts already exist in the tenant by name.
    .DESCRIPTION
        Reads JSON files from a specified folder and checks whether a script with the same
        'displayName' property already exists in the tenant via Microsoft Graph. Returns any matches
        so the caller can decide how to handle conflicts.
    .PARAMETER FolderPath
        Mandatory. The path to the folder containing JSON files of Intune Device Health Scripts.
    .NOTES
        Requires:
        - Microsoft.Graph PowerShell SDK (Invoke-MgGraphRequest, Get-MgContext)
        - DeviceManagementConfiguration.Read.All (minimum)
    .EXAMPLE
        Test-IntuneDeviceHealthScript -FolderPath "C:\temp\IntuneHealthScripts"
    .LINK
    ...
    foreach ($File in $JsonFiles) {
        ...
        $FilterUri = "https://graph.microsoft.com/beta/deviceManagement/deviceHealthScripts?`$filter=displayName eq '$PolicyName'"
